...
#> 1 sqrt(x)       644ns    757ns   655851.      848B        0
#> 2 x^0.5        2.85µs   3.02µs   309232.      848B        0
```
...
#> 1 sqrt(x)       644ns    757ns   655851.
#> 2 x^0.5        2.85µs   3.02µs   309232.
```
...
As with all microbenchmarks, pay careful attention to the units: here, each computation takes about 640 ns, 640 billionths of a second. To help calibrate the impact of a microbenchmark on run time, it's useful to think about how many times a function needs to run before it takes a second. If a microbenchmark takes:
...
The `sqrt()` function takes about 640 ns, or 0.64 µs, to compute the square roots of 100 numbers. That means if you repeated the operation a million times, it would take 0.64 s, and hence changing the way you compute the square root is unlikely to significantly affect real code. This is the reason you need to exercise care when generalising microbenchmarking results.
